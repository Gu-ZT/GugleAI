...
- 更新或整理发布说明时，读取 Git tags 并按基础语义化版本归并；忽略 `+build.N` 构建后缀，同一版本的多个构建合并为一条记录。
- 变更日志同时维护 `CHANGES.md` 和 `CHANGES.en.md`，中文与英文条目保持对应。
- 已发布版本按版本号记录；当前 tags 之后尚未发布的提交归入 `未发布`/`Unreleased` 区段。
- 未发布内容可以使用当前项目版本号和当前日期作为标题，格式为 `## [<version>] - YYYY-MM-DD`；版本号按“版本与发布”中的配置读取并保持一致。
- 仅修改变更日志文件及完成任务所必需的内容，不要改动其他项目文件或用户已有的无关变更。
