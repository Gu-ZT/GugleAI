...
- **提示词变量**:智能体系统提示词支持 `{{date}}`、`{{time}}`、`{{datetime}}`、`{{system}}`、`{{arch}}`、`{{language}}`、`{{model_name}}` 和 `{{username}}`,请求发出前按当前环境动态替换
...
- **无尽画布**:支持无限平移和缩放;文字生成会创建并连接新的文字子节点;生成时只读取直接相连的上一级节点,文字节点内容加入提示词,图像节点作为参考图;每个参考图节点只保存一张图片并隐藏无关的模型设置
...
4. 无尽画布中文字节点生成后会新增文字子节点;每个含图片的节点只保存一张参考图,可替换图片或连接多个参考节点到新的空图像节点进行生成
