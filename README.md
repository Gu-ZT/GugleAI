...
- **自定义连接**:原侧栏配置集中到设置弹出层;API 端点和 API Key 成对保存为连接配置,可从同一个下拉菜单切换,并通过独立弹出层添加或编辑每个连接的可用模型列表;Key 仅显示脱敏摘要;端点不带 `/v1` 时自动补全
...
前端使用 Vue Router Hash History 管理 `/image`、`/chat`、`/canvas` 三个工作区,页面分别位于 `src/views/`。设置、连接编辑和预览交互弹层位于 `src/components/modals/`。Arco Design 组件通过 `unplugin-vue-components` 按需解析;公共逻辑分别位于 `src/api/index.ts`、`src/chat/index.ts`、`src/canvas/index.ts` 和 `src/router/index.ts`,其中 API 连接、聊天会话和画布图结构使用类封装。实际网络请求仍统一经过 `src/App.vue` 的 Tauri HTTP `fetch` 封装。
...
1. 使用左侧图标切换「生图」「聊天」或「无尽画布」;点击左下角设置按钮配置端点、Key 和可用模型
