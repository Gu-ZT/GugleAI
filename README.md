...
- **多工作区**:顶部可在图像生成、文字聊天和无尽画布之间切换,窗口与产品名称统一为 GugleAI
- **自定义连接**:API 端点和 API Key 成对保存为连接配置,可从同一个下拉菜单切换,并通过弹出层添加或编辑每个连接的可用模型列表;Key 仅显示脱敏摘要;端点不带 `/v1` 时自动补全
...
前端使用 Vue Router Hash History 管理 `/image`、`/chat`、`/canvas` 三个工作区。Arco Design 组件通过 `unplugin-vue-components` 按需解析;公共逻辑分别位于 `src/api/index.ts`、`src/chat/index.ts`、`src/canvas/index.ts` 和 `src/router/index.ts`,其中 API 连接、聊天会话和画布图结构使用类封装。实际网络请求仍统一经过 `src/App.vue` 的 Tauri HTTP `fetch` 封装。
...
1. 在顶部切换「图像生成」「聊天」或「无尽画布」;左侧 API 连接可通过弹出层配置端点、Key 和可用模型
2. 图像生成模式可输入提示词并添加参考图;聊天模式可选择或输入文字模型后进行连续对话
