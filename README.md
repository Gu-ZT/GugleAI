...
- **自定义连接**:API 端点、API Key、模型 ID 均可自由配置,自动保存;端点不带 `/v1` 时自动补全
...
- **请求重试**:可按 HTTP 错误码自动重试生成请求,错误码和重试次数均可配置
...
1. 在左侧设置栏填入 API 端点(如 `https://api.openai.com/v1` 或中转地址)、API Key 和模型 ID,也可以直接粘贴连接配置
