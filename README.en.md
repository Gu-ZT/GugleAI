...
- **Organized settings**: the Settings page has nested routes for Model Settings, Agent Settings, General Settings, Backups, and full-size Logs; entering Settings from the main navigation always opens Model Settings, which adds a secondary provider sidebar
...
- **Backup and restore**: package settings, agents, conversations, preview history, and Infinite Canvas images into one ZIP; create, schedule, import, export, and delete backups, calculate automatic intervals from the previous backup creation time, retain only the configured number of automatic backups, and optionally upload or download through WebDAV
